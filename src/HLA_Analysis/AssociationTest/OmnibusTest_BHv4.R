...
data.covar = args[4]
outfile=args[5]
phen.name=args[6] ## phenotype name defined in .phe file
rare.thres=as.numeric(args[7]) ## frequency cut-off to ignore alleles in test
...
# (3) pheno
pheno <- as.numeric(as.matrix(read.table(phenfile, header=T))[, phen.name])-1
#mode(pheno)="numeric"
...
outfilename = paste(outfile, phen.name, paste0(condvar,collapse='+'), "omnibus", sep='.', collapse='.')
