...
from HLA_Analysis.HLA_Analysis import HATK_LogisticRegression, HATK_OmibusTest
...
HLA_names = ["A", "B", "C", "DPA1", "DPB1", "DQA1", "DQB1", "DRB1"] # => This has to move into 'HLA_Study' class.
...
def ArgRouter(_args):
    ...
    if _args.input:
        ...
        * --input := the common prefix to point out next files
        ...
        if not _args.covar:
            _args.covar = _args.input + ".covar" if os.path.exists(
                _args.input + ".covar") else _args.input + ".cov" if os.path.exists(_args.input + ".cov") else None
    ...
    if f_sum == 0:
        ...
        if _args.chped:
            ...
            if _args.chped == -1:
                print(std_ERROR_MAIN_PROCESS_NAME + "Failed to process HPED file('{}') by NomenCleaner.".format(
                    _args.hped))
                sys.exit()
